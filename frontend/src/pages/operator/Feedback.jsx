...
const Feedback = () => {
  ...
  const getReviewBorderClass = (rating) => {
    if (rating >= 4) return 'border-green-300';
    if (rating === 3) return 'border-yellow-300';
  ...
  const recentReviews = filteredReviews.filter((review) => review.rating >= 3);
  const lowRatingReviews = (feedbackData?.reviews || []).filter((review) => review.rating <= 2);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1 ml-4">Monitor and manage customer reviews</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">Recent Reviews ({recentReviews.length})</h3>
        ...
        {recentReviews.length === 0 ? (
          ...
          recentReviews.map((review) => (
            <div
              key={review.id}
              className={`bg-white rounded-xl p-6 shadow-sm border-2 ${getReviewBorderClass(review.rating)} hover:shadow-md transition-shadow cursor-pointer`}
              onClick={() => {
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          {lowRatingReviews.map((review, index) => (
            <div key={`low-rating-${review.id || index}`} className="flex items-start gap-3 p-3 bg-red-50 border border-red-200 rounded-lg">
              <div className="p-2 rounded-full bg-red-100 text-red-600">
                <AlertCircle className="h-4 w-4" />
              </div>
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-900">{review.comment || 'Low rating feedback reported'}</p>
                <p className="text-xs text-gray-500 mt-1">
                  {review.stationName} • {review.rating}⭐
                </p>
              </div>
            </div>
          ))}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className={`inline-flex mt-1 px-2 py-0.5 rounded-full border text-xs font-medium ${getChargingBadgeClass(selectedFeedback.chargerType)}`}>
                  {getChargingLabel(selectedFeedback.chargerType)}
                </p>
              </div>
              <div>
